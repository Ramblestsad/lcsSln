...
- `Realtime/`: SignalR chat hub, client contract, DTOs, and room state adapter.
...
- After finding the wiring point, read the matching runtime code in the feature folder (`Auth/`, `Todos/`, `Orders/`, `Engagement/`, `Realtime/`, or `Greeter/`).
...
- Redis-backed features: start with `Infrastructure/Extensions/RedisServiceCollectionExtensions.cs`, `Engagement/`, and `Realtime/`.
- SignalR or chat work: start with `Infrastructure/Extensions/SignalRServiceCollectionExtensions.cs` and `Realtime/ChatHub.cs`.
