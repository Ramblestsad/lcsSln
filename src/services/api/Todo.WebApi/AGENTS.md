...
This repository scope here is the `src/services/api/Todo.WebApi` API and its DAL dependency (`src/services/api/Todo.DAL`). Key paths:
...
- `dotnet run --project src/services/api/Todo.WebApi/Todo.WebApi.csproj`: run the API using the default profile.
- `dotnet run --project src/services/api/Todo.WebApi/Todo.WebApi.csproj --launch-profile Local`: run with the `Local` profile.
- `dotnet test tests/services/api/Todo.WebApi.Tests/Todo.WebApi.Tests.csproj`: run API tests.
...
## Testing Guidelines
The API test project lives at `tests/services/api/Todo.WebApi.Tests/`.
- Naming: `*Tests.cs` per subject.
- Run API tests with `dotnet test tests/services/api/Todo.WebApi.Tests/Todo.WebApi.Tests.csproj` from the solution root.
