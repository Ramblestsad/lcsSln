...
// (2^nd node in A and 3^rd node in B) are different node references. In other words, they point to
// two different locations in memory, while the nodes with value 8 in A and B (3^rd node in A and
// 4^th node in B) point to the same location in memory.
//
// Example 2:
//
